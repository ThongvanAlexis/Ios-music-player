...
Working title only; the app has no final name yet. This project definition is a draft while the remaining product and workflow choices are being discussed.
...
- **Platform:** Native iOS; older iPhones must be supported. Exact minimum iOS version remains a proposal until reviewed.
...
- **Themes:** ZIP archives with JSON and images; the base skin must be exportable. Whether skins can rearrange layouts is pending user input.
...
| Importable ZIP skins and exportable base skin | Users should be able to create and share visual styles | Confirmed; layout freedom pending |
| Support older iPhones | Compatibility must extend beyond the user's current device | Confirmed; minimum version pending |
...
- Behavior at the end of a saved passage: stop, continue the track, or repeat by default. Asked; no default selected yet.
- Whether themes can change screen layouts or only restyle existing components. Asked; no default selected yet.
- Workflow settings, including phase size, execution mode, and parallel agents. Proposed settings sent; awaiting answer before writing config.
- Exact minimum iOS version and representative older-device test coverage.
- Public versus personal distribution, and access/budget for hosted macOS builds and signing.
- Whether JSON export must also support restore/import. A round-trip restore is recommended for a useful backup, but was not explicitly requested.
- Whether tag exploration means embedded artist/album/genre metadata, custom user tags, or both.
...
---
*Last updated: 2026-09-12 after initial project questioning; draft pending remaining decisions.*
